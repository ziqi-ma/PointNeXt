...
class ShapeNetPartNormal(Dataset):
    ...
    def __init__(self,
        ...
        self.all_rotation = torch.load(f"/data/ziqi/shapenetpart/random_rotation_test.pt")
    ...
    def __getitem__(self, index):
        cur_rot = self.all_rotation[index,:]
        ...
        if self.apply_rotation:
            pos = rotate_pts(torch.tensor(pos), cur_rot).numpy()
            normal = rotate_pts(torch.tensor(normal), cur_rot).numpy()
...
@DATASETS.register_module()
class ShapeNetPartEstimateNormal(Dataset):
    classes = ['airplane', 'bag', 'cap', 'car', 'chair',
               'earphone', 'guitar', 'knife', 'lamp', 'laptop',
               'motorbike', 'mug', 'pistol', 'rocket', 'skateboard', 'table']
    seg_num = [4, 2, 2, 4, 4, 3, 3, 2, 4, 2, 6, 2, 3, 3, 3, 3]
    ...
    def __init__(self,
        ...
        seg_list = []
        rotation_list = []
        for class_choice in self.classes:
            id_choice = self.cat2id[class_choice]
            indices = (all_label == id_choice).squeeze()
            cur_data = all_data[indices]
            cur_label = all_label[indices]
            cur_seg = all_seg[indices]
            cur_rotation = all_rotation[indices]
            if subset:
                # get subset
                subset_idxs = np.loadtxt(f"/data/ziqi/shapenetpart/{class_choice}_subsample.txt").astype(int)
                cur_data = cur_data[subset_idxs]
                cur_label = cur_label[subset_idxs]
                cur_seg = cur_seg[subset_idxs]
                cur_rotation = cur_rotation[subset_idxs]
            data_list.append(cur_data)
            label_list.append(cur_label)
            seg_list.append(cur_seg)
            rotation_list.append(cur_rotation)
        
        self.data = np.concatenate(data_list, axis=0)
        self.label = np.concatenate(label_list, axis=0)
        self.seg = np.concatenate(seg_list, axis=0)
        self.rotation = np.concatenate(rotation_list, axis=0)
    ...
    def __getitem__(self, index):
        ...
        cur_rotation = self.rotation[index]
        seg = self.seg[index].astype(np.int64)
        
        xyz = self.data[index]
        if self.apply_rotation:
            xyz = rotate_pts(torch.tensor(xyz), torch.tensor(cur_rotation))
